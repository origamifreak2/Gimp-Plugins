...
def sixteen_by_nine(_image, _drawable):

    # create duplicate layer to use as foreground
    forground = _drawable.copy()
    _image.add_layer(forground, 0)

    # get canvas width and height
    canvas_width = pdb.gimp_image_width(_image)
    canvas_height = pdb.gimp_image_height(_image)
    ...
    pdb.gimp_image_resize(_image, canvas_width, canvas_height, off_x, off_y)

    # resize background
    pdb.gimp_layer_scale(_drawable, bg_width, bg_height, True)

    # blur background
    pdb.plug_in_gauss(_image, _drawable, 90, 90, 0)
